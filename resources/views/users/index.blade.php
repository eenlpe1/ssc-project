...
<div class="p-4 sm:p-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6">
        <h2 class="text-2xl font-bold mb-3 sm:mb-0">USERS</h2>
        @if(Auth::user()->canCreateUsers())
            <button id="addUserBtn" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-all duration-300 w-full sm:w-auto justify-center sm:justify-start">
    ...
    <div class="bg-white rounded-lg shadow mb-6">
        <div class="flex flex-wrap border-b">
            <a href="{{ route('users.index') }}" 
               class="px-3 sm:px-6 py-3 text-sm sm:text-base {{ $currentRole === 'all' ? 'text-blue-600 border-b-2 border-blue-600 font-medium' : 'text-gray-500 hover:text-blue-600 hover:border-b-2 hover:border-blue-600' }}">
                All
            </a>
            @if(Auth::user()->role === 'admin')
            <a href="{{ route('users.index', ['role' => 'admin']) }}" 
               class="px-3 sm:px-6 py-3 text-sm sm:text-base {{ $currentRole === 'admin' ? 'text-blue-600 border-b-2 border-blue-600 font-medium' : 'text-gray-500 hover:text-blue-600 hover:border-b-2 hover:border-blue-600' }}">
                Admin
            </a>
            @endif
            <a href="{{ route('users.index', ['role' => 'adviser']) }}" 
               class="px-3 sm:px-6 py-3 text-sm sm:text-base {{ $currentRole === 'adviser' ? 'text-blue-600 border-b-2 border-blue-600 font-medium' : 'text-gray-500 hover:text-blue-600 hover:border-b-2 hover:border-blue-600' }}">
                Adviser
            </a>
            <a href="{{ route('users.index', ['role' => 'user']) }}" 
               class="px-3 sm:px-6 py-3 text-sm sm:text-base {{ $currentRole === 'user' ? 'text-blue-600 border-b-2 border-blue-600 font-medium' : 'text-gray-500 hover:text-blue-600 hover:border-b-2 hover:border-blue-600' }}">
                User
            </a>
        </div>
    </div>
            ...
            <table class="w-full whitespace-nowrap">
                <thead>
                    <tr class="bg-[#1e3a8a] text-white">
                        <th class="px-3 sm:px-6 py-3 sm:py-4 text-center">#</th>
                        <th class="px-3 sm:px-6 py-3 sm:py-4 text-left">Name</th>
                        <th class="px-3 sm:px-6 py-3 sm:py-4 text-left hidden sm:table-cell">Email</th>
                        <th class="px-3 sm:px-6 py-3 sm:py-4 text-left hidden md:table-cell">Position</th>
                        <th class="px-3 sm:px-6 py-3 sm:py-4 text-center">Role</th>
                        <th class="px-3 sm:px-6 py-3 sm:py-4 text-center hidden lg:table-cell">Created At</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($users as $index => $user)
                        <tr class="hover:bg-gray-50 transition-colors duration-200 cursor-pointer" onclick="showUserDetails({{ $user->id }})">
                            ...
                            <td class="px-3 sm:px-6 py-3 sm:py-4 font-medium text-sm sm:text-base">{{ $user->name }}</td>
                            <td class="px-3 sm:px-6 py-3 sm:py-4 text-sm sm:text-base hidden sm:table-cell">{{ $user->email }}</td>
                            <td class="px-3 sm:px-6 py-3 sm:py-4 text-sm sm:text-base hidden md:table-cell">{{ $user->position ?? 'Not specified' }}</td>
                            ...
                            <td class="px-3 sm:px-6 py-3 sm:py-4 text-center text-sm sm:text-base hidden lg:table-cell">{{ $user->created_at->format('M d, Y') }}</td>
                            <td class="px-3 sm:px-6 py-3 sm:py-4 text-center" onclick="event.stopPropagation()">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-3 sm:px-6 py-4 text-center text-gray-500">No users found</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="bg-white rounded-lg w-full max-w-md p-4 sm:p-6 max-h-[90vh] overflow-y-auto">
        ...
        <form id="editUserForm" method="POST">
            ...
            <div class="mt-6 flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-3">
                <button type="button" class="w-full sm:w-auto px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50 order-last sm:order-first" onclick="closeEditModal()">Cancel</button>
                <button type="submit" class="w-full sm:w-auto px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Update User</button>
            </div>
        </form>
    </div>
</div>
...
<div id="resetPasswordModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 p-4">
    <div class="bg-white rounded-lg w-full max-w-md p-4 sm:p-6 max-h-[90vh] overflow-y-auto">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg sm:text-xl font-bold">Reset Password</h3>
        ...
        <form id="resetPasswordForm" method="POST">
            ...
            <div class="mt-6 flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-3">
                <button type="button" class="w-full sm:w-auto px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50 order-last sm:order-first" onclick="closeResetPasswordModal()">Cancel</button>
                <button type="submit" class="w-full sm:w-auto px-4 py-2 bg-yellow-600 text-white rounded-md hover:bg-yellow-700">Reset Password</button>
            </div>
        </form>
    </div>
</div>
...
<div id="addUserModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 p-4">
    <div class="bg-white rounded-lg w-full max-w-md p-4 sm:p-6 max-h-[90vh] overflow-y-auto">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg sm:text-xl font-bold">Add New User</h3>
        ...
        <form action="{{ route('users.store') }}" method="POST">
            ...
            <div class="mt-6 flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-3">
                <button type="button" class="w-full sm:w-auto px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50 order-last sm:order-first" onclick="closeModal()">Cancel</button>
                <button type="submit" class="w-full sm:w-auto px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Create User</button>
            </div>
        </form>
    </div>
</div>
...
<!-- User Details Modal -->
<div id="userDetailsModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full p-4">
    <div class="relative top-20 mx-auto p-4 sm:p-5 border w-full sm:w-96 shadow-lg rounded-md bg-white">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg sm:text-2xl font-bold">User Details</h3>
        ...
        <div class="mt-4">
            <div class="flex flex-col items-center mb-6">
                <div class="w-24 h-24 sm:w-32 sm:h-32 rounded-full overflow-hidden bg-gray-100 mb-4">
                ...
                <h4 id="userName" class="text-lg sm:text-xl font-semibold mb-1"></h4>
                <span id="userRole" class="px-3 py-1 rounded-full text-xs sm:text-sm font-medium"></span>
            </div>
            <div class="space-y-4">
                <div>
                    <h4 class="text-sm font-medium text-gray-500">Email</h4>
                    <p id="userEmail" class="text-gray-900 text-sm sm:text-base break-words"></p>
                </div>
                <div>
                    <h4 class="text-sm font-medium text-gray-500">Nickname</h4>
                    <p id="userNickname" class="text-gray-900 text-sm sm:text-base"></p>
                </div>
                <div>
                    <h4 class="text-sm font-medium text-gray-500">Department</h4>
                    <p id="userDepartment" class="text-gray-900 text-sm sm:text-base"></p>
                </div>
                <div>
                    <h4 class="text-sm font-medium text-gray-500">Position</h4>
                    <p id="userPosition" class="text-gray-900 text-sm sm:text-base"></p>
                </div>
                <div>
                    <h4 class="text-sm font-medium text-gray-500">Member Since</h4>
                    <p id="userCreatedAt" class="text-gray-900 text-sm sm:text-base"></p>
                </div>
            </div>
        </div>
    </div>
    ...
    function showUserDetails(userId) {
        fetch(`/users/${userId}`)
            ...
            .then(user => {
                ...
                document.getElementById('userRole').className = `px-3 py-1 rounded-full text-xs sm:text-sm font-medium ${
